...
from Homeworks.homework_5.task_1.models import TaskBase, db1, Task, Base, engine
...
async def read_root(task_id: int):
    ...
    return f"GET запрос: Task:  task_id: {task.task_id}, title: {task.title}, " \
...
async def delete_item(task_id: int):
    ...
    for task in tasks:
        db1.delete(task)
        db1.commit()
    return {f'DELETE "task_id": {task_id}'}
